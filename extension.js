...
const WindowContextMenu = new Lang.Class({
  ...
  _updateMinimizeItem: function() {
    this._minimizeItem.label.text = this._metaWindow.minimized ? _(
        "Unminimize") :
      _("Minimize");
  },
...
const WindowTitle = new Lang.Class({
  ...
  _updateTitle: function() {
    ...
    if (this._metaWindow.minimized)
      ...
      this.label_actor.text = myTitle;
  },

  _updateIcon: function() {
    let app = Shell.WindowTracker.get_default()
      .get_window_app(this._metaWindow);
    if (app)
...
const AppButton = new Lang.Class({
  ...
  _updateVisibility: function() {
    if (!this._perMonitor) {
      ...
      this.actor.visible = this.getWindowList()
        .length >= 1;
    }
  },
  ...
  getWindowList: function() {
    return this.app.get_windows()
      .filter(Lang.bind(this, function(win) {
...
const WorkspaceIndicator = new Lang.Class({
  ...
  _init: function() {
    ...
    this._currentWorkspace = global.screen.get_active_workspace()
      .index();
    ...
    this._screenSignals.push(global.screen.connect_after(
      'workspace-switched', Lang.bind(this, this._updateIndicator)));
    ...
    this._settingsChangedId = this._settings.connect(
      'changed::workspace-names', Lang.bind(this, this._updateMenu));
  },
  ...
  _updateIndicator: function() {
    this.workspacesItems[this._currentWorkspace].setOrnament(PopupMenu.Ornament
      .NONE);
    this._currentWorkspace = global.screen.get_active_workspace()
      .index();
    this.workspacesItems[this._currentWorkspace].setOrnament(PopupMenu.Ornament
...
const WindowList = new Lang.Class({
  ...
  _init: function(perMonitor, monitor) {
    ...
    indicatorsBox.add(this._workspaceIndicator.container, {
      expand: false,
      y_fill: true
    });
    ...
    this._workspacesOnlyOnPrimaryChangedId =
      this._workspaceSettings.connect(
    ...
    Main.ctrlAltTabManager.addGroup(this.actor, _("Window List"),
      'start-here-symbolic');
    ...
    this._fullscreenChangedId =
      global.screen.connect('in-fullscreen-changed', Lang.bind(this,
        function() {
          this._updateKeyboardAnchor();
        }));
  ...
  _getWorkspaceSettings: function() {
    ...
    if (settings.list_keys()
      .indexOf('workspaces-only-on-primary') > -1)
      return settings;
...
const Extension = new Lang.Class({
  ...
  _buildWindowLists: function() {
    ...
    let showOnAllMonitors = this._settings.get_boolean(
